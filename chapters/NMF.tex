...
Finally, in section \ref{two applications of NMF section}, two applications of NMF are presented: feature extraction from a dataset of grey scale faces images and topic modeling from a collection of documents.
\section{Definition}
...
Their work is especially famous because they  demonstrate an algorithm for non-negative matrix factorization that is able to learn parts of faces, in contrast to other decomposition methods like PCA or SVD that learn holistic features. Their results are also reproduced in this thesis in subsection \ref{Example feature extraction NMF} to show a prominent example of NMF's application.
...
In this restricted exact version of the problem, it naturally arises the definition of non-negative rank and the exact-NMF is used for (1) geometric interpretation of the NMF problem, and (2) to study the computational complexity of the NMF problem, the result is, in short, that the NMF belongs to the class of NP-hard problems, as showed by Vavasis \cite{vavasis2007complexitynonnegativematrixfactorization}. 
