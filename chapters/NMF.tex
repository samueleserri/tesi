...
\end{equation}
To provide a nice Visualization we use this observation: $V = WH \iff VD_V = (WD_W)D_W^{-1}HD_V$ for any invertible diagonal matrices $D_V \in \R^{n \times n}$ and $D_W \in \R^{r \times r}$ with positive diagonal entries.
If we choose $D_V$ and $D_W$ such that the columns of $VD_V$ and $WD_W$ are normalized to sum to one, then the convex cones $\text{cone}(VD_V)$ and $\text{cone}(WD_W)$ intersect the unit simplex in $\R^m$ in two convex polytopes.
This allows one to visualize the NMF problem as finding a polytope with $r$
\par
The property that all columns of $V$ sum to one is called column-stochaasticity and can be expressed as follows:
\begin{equation}
    \mathbf{1}^\top V = \mathbf{1}^\top
\end{equation}
