...
the Exact NMF formulation has a nice \textit{geometric interpretation} that will be discussed in section \ref{Geometric interpretation section}, since it can be insightful to construct a geometric understanding of the problem.
\par
...
\end{figure}
The separable NMF represent an important class of models in applications \cite{arora2012computing}, \cite{gillis2013fast}; for example in text mining \cite{arora2013practical}, 
...
\par
There are other sufficient conditions that are not included in this thesis, for instance the so called \textit{sufficiently scattered condition} (SCC), see \cite{gillis2024checking} for the formulation and an example on how to check it with a global non-convex optimization software; additionally the interested reader may refer to \cite{huang2013non} for a geometrical perspective on uniqueness results.
...
The reason for this is that because it is easier to visualize and because there is a close connection between the NMF problem in computational geometry known as the and the nested polytope problem (NPP)  \cite{donoho2003does}.
%% For an overview on the NPP see: \cite{dobbins2019universalitytheoremnestedpolytopes}.
A detailed discussion on the geometric interpretation of NMF can be found in \cite{gillis2020nonnegative} chapter $2.1$ and in \cite{fu2019nonnegative}.
