\chapter{Non-Negative Matrix Factorization}\label{NMF CHAPTER}
...
\par
In section \ref{Metrics section}, the most common metrics used in the NMF literature are discussed, specifically the Frobenius norm, the Kullback-Leibler divergence, and the $\beta$-divergence.
...
Then, section \ref{identifiability section} discusses the identifiability problem of NMF, that is, the non-uniqueness of the solutions of the exact NMF problem in general.
Section \ref{Geometric interpretation section} concerns the geometric interpretation of NMF, in particular a linear dimensionality reduction perspective is given along with a brief overview of the geometric interpretation with convex cones.
Finally, in section \ref{two applications of NMF section}, two applications of NMF are presented: feature extraction from a dataset of gray-scale faces images and topic modeling from a collection of documents.
\section{Definition}
...
\end{remark}
After solving the optimization problem (\ref{NMF optimization problem}), as result we obtain two matrices $W \in \R_+^{m \times r}$ and $H \in \R_+^{r \times n}$ such that their product $WH \in \R_+^{m \times n}$ is an approximation to the original matrix $V \in \R_+^{m \times n}$.
\par
The columns of the original non-negative matrix $V \in R^{m \times n}_+$ are then approximated as non-negative linear combinations of the columns of $W$ weighted by the coefficients in the corresponding column of $H$:
...
As we will see in chapter \ref{ALGO CHAPTER} the Frobenius norm is the most widely used metric in NMF algorithms. In addition, here we also present the Kullback-Leibler divergence and the $\beta$-divergence, which generalizes both.
\par
Ideally, the choice of the distance measure $D$ should be made according to the distribution of the data contained in the matrix $V$ and the noise that affects it. However, since in practice these distributions are often unknown, assumptions need to be made.
In subsection \ref{Frobenius norm section} we show that the Frobenius norm is the maximum likelihood estimator when the data is affected by i.i.d. Gaussian noise, which is a common and reasonable assumption in many applications.
...
\end{equation}
where $R_W$ and $R_H$ are regularization functions that promote some desired properties in the factors $W$ and $H$, and $\alpha_W$ and $\alpha_H$ are non-negative hyperparameter that control the strength of the regularization.
...
See \cite{gillis2020nonnegative} subsection 5.1.1 for a comprehensive discussion on the error measures associated with various distributions.
...
\begin{figure}
    ...
    \caption{Plot of the $\beta$-divergence $d_{\beta}(1,x)$ for $\beta$ values of $0$, $1$ $\frac{1}{2}$, $2$, and $3$.}
...
the Exact NMF formulation has a nice \textit{geometric interpretation} that will be discussed in section \ref{Geometric interpretation section}.
\par
...
\par
An interesting example of a matrix whose non-negative rank is strictly greater than its rank was presented by Thomas in \cite{thomas1974rank}:
\begin{example}
...
\par
Even after removing this scaling ambiguity, one should not expect to find an unique exact NMF solution in general.
In the literature this is also known as the \textit{identifiability} problem of NMF. The interested reader may refer to \cite{gillis2020nonnegative} chapter $4$ for a detailed discussion on this topic.
\par
In this section we examine which necessary condition is required for identifiability and present a sufficient condition for it.
...
\begin{equation}
    ...
    \end{bmatrix}.
\end{equation}
We can see that the support of the first column of $W$ is contained in the support of the second column of $W$:
\begin{equation}
    \text{supp}(W[:,1]) \subseteq \text{supp}(W[:,2])
\end{equation}
thus, we can construct another exact NMF of $WH$ as follows:
\begin{equation}    \tilde{W} = \begin{bmatrix}
    2 & \textcolor{blue}{5} & 0 \\
    1 & \textcolor{blue}{2} & 3 \\
    2 & \textcolor{blue}{4} & 1 \\
    0 & \textcolor{blue}{4} & 1 \\
    ...
    \end{bmatrix}.
\end{equation}
where $\tilde{W}[:,2] = W[:,2] + 1 \cdot W[:,1]$ and $\tilde{H}[1,:] = H[1,:] - 1 \cdot H[2,:]$.
We can verify that $\tilde{W} \tilde{H} = WH$ is indeed another exact NMF of $WH$.
\end{example}
...
Let $V \in \R_+^{m \times n}$ be a non-negative matrix and let $V = WH$ be an exact NMF of $V$ of size $r$.
If the matrices $W$ and $H$ contain non-singular diagonal sub-matrices of size $r \times r$, then the exact NMF solutions are identifiable.
\end{theorem}
\begin{proof}
...
An exact NMF $V = WH$ is said to be separable if there exists a set of $r$ indices $\{j_1, j_2, \ldots, j_r\} \subseteq \{1, 2, \ldots, n\}$ such that:
\begin{equation}
...
\end{figure}
The separable NMF represent an important class of models in applications \cite{gillis2013fast}; for example in text mining \cite{arora2013practical}, 
...
\par
There are other sufficient conditions that are not included in this thesis, for instance, the so called \textit{sufficiently scattered condition} (SCC). The interested reader may refer to \cite{huang2013non} for a geometrical perspective on uniqueness results.
...
Because the topic is so expansive, this section provides only a brief overview. For a more detailed discussion on the geometric interpretation of NMF see \cite{gillis2020nonnegative} chapter $2$ and the references therein.
...
\subsection{Geometric interpretation with convex cones}
In linear algebra a cone is a subset of a vector space that is closed with respect to the operation of positive scalar multiplication.
\begin{definition}[Cone]
A set $C \subseteq \R^m$ is a cone if for any $x \in C$ and any non-negative scalars $\alpha \geq 0$ it holds that:
\begin{equation}
    \alpha x \in C.
\end{equation}
\end{definition}
A convex cone is a subset of a vector space that is closed under linear combinations with positive coefficients.
\begin{definition}[Convex cone]
    A set $C \subseteq \R^m$ is a convex cone if it is a cone and additionally closed under addition. That is, for any $x, y \in C$ and any non-negative scalars $\alpha, \beta \geq 0$ it holds that:
    \begin{equation}
        \alpha x + \beta y \in C.
    \end{equation}
\end{definition}
...
\begin{equation}
    V[:,j] = WH[:,j], \quad \forall j = 1, \ldots, n.
\end{equation}
...
The reason for this is that it is easier to visualize and because there is a close connection between the NMF and a problem in computational geometry known as the and the nested polytope problem (NPP)  \cite{donoho2003does}.
...
the first modern definition of the NMF problem is usually attributed to the work of Paatero and Tapper in 1994 \cite{paatero1994positive} where they defined the so-called positive matrix factorization (PMF) problem, a particular case of NMF where the distance measure used is the Frobenius norm.
...
Their work is especially famous because they  demonstrate an algorithm for non-negative matrix factorization that is able to learn parts of faces, in contrast to other decomposition methods like SVD/PCA that learn holistic features. Their results are also reproduced in this thesis in subsection \ref{Example feature extraction NMF} to show a prominent example of NMF's application.
...
The dataset used in this example is the CBCL face dataset \footnote{See appendix \ref{code documentation appendix} for data availability and details on the code used}.
...
\begin{figure}
    \centering
    \includegraphics[width=1\textwidth]{Imgs/CBCL_colums_of_W.png}
...
\begin{figure}
    \centering
    \includegraphics[width=0.75\textwidth]{Imgs/Reconstruction_1.png}
...
\begin{table}[htbp]
\centering
\caption{Top 10 words extracted by NMF for some of the 30 topics in the TDT2 corpus}
...
\par
This is an example of application of a sparse NMF model, since most words do not appear in most documents most entries of the input matrix $V$ are zero.
...
In this case there exists very efficient algorithms to find the NMF solution, see \cite{arora2013practical} for a practical algorithm.
...
An NMF model requires to choose a distance measure and a reconstruction rank; NMF models can be extremely versatile and can be adapted to different applications by choosing the appropriate distance measure, possibly with regularization terms, and approximation rank.
To better understand the problem is useful to analyze the exact version of the NMF, known as the exact NMF problem, here the goal is to find two non-negative matrices $W$ and $H$ such that their product is exactly equal to the original data matrix $V$.
In this restricted exact version of the problem, it naturally arises the definition of non-negative rank and the exact-NMF is used for (1) geometric interpretation of the NMF problem, (2) identifiability of the solutions, and (3) to study the computational complexity of the NMF problem, the result is, in short, that the NMF belongs to the class of NP-hard problems, as showed by Vavasis \cite{vavasis2007complexitynonnegativematrixfactorization}. 
In the same paper, Vavasis also showed that a polynomial-time local search heuristic exists. In the next chapter we will see the most important algorithms to undertake the NMF problem, and it will be clear that they come with no guarantees of global optimality.
