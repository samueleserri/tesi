...
\par
In section \ref{Synthetic data section} is explained how the synthetic data are generated and which numerical simulations are run on it, following the literature, specifically the approach of N. Gillis \cite{gillis2020nonnegative}, we distinguish between the dense and the sparse case.
The two main plots of this section are shown in figure \ref{fig:Random init comparison} and figure \ref{fig:Sparse uniform data random init}.
In section \ref{init comparison section} the HALS algorithm is used to compare the influence of the initialization strategy on the convergence speed and the final relative reconstruction error, in particular we fit the same NMF model on the same data changing only the initialization method, the results are shown in figure \ref{fig:Hals_init_comparison}.
The section \ref{low rank reconstruction section} is dedicated to the low rank reconstruction experiments, where the reconstruction rank $k$ is much smaller than the rank of the matrix $V$, $rank(V) = r$. 
Finally, in section \ref{CBCL dataset section} benchmark datasets are used to compare the performance of the algorithms, in particular the CBCL face dataset and the Olivetti faces dataset for the dense case and the tdt2 documents corpus for the sparse case, the results for the CBCL dataset are shown in figure \ref{fig:CBCL dataset} for the CBCL dataset, figure \ref{fig:Top30 news dataset} for the tdt2 dataset and for the Olivetti faces dataset in figure in the appendix \ref{fig:Olivetti dataset}.
\begin{table}[ht]
\centering
\begin{tabular}{lllll}
\hline
...
\multicolumn{1}{|l|}{rand U{[}0,1{)}} & \multicolumn{1}{l|}{250} & \multicolumn{1}{l|}{100}  & \multicolumn{1}{l|}{100}   & \multicolumn{1}{l|}{0\%}      \\ \hline
...
We generate a matrix $V \in \R^{m \times n}$ as $V = WH$ where $W \in \R^{m \times r}$ and $H \in \R^{r \times n}$, with $m = 250,  n = 100$, and $r = 20$.
The entries of $W$ and $H$ are sampled from a uniform distribution over the interval $[0,1)$. This matrix is generated using the method \texttt{numpy.random.rand}\footnote{{\url{https://numpy.org/doc/stable/reference/random/generated/numpy.random.rand.html}}} from the numpy library; the seed is set to $15$ for reproducibility.
...
We then run the MU algorithm with the Frobenius norm \ref{MU for Frobenius}, the Hierarchical Alternating Least Squares (HALS) algorithm \ref{HALS}, the Alternating Least Squares (ALS) algorithm \ref{ALS}, and the Projected Gradient Descent (PGD) algorithm \ref{Projected Gradient} for $1000$ iterations.
...
The results for the dense case are shown in figure \ref{fig:Random init comparison}, and in figure \ref{fig:Sparse uniform data random init} for the sparse case.
\par
...
In order to reduce the variance of the results due to random initialization, we run each algorithm $10$ times with $10$ different random initializations and we average the relative reconstruction error over those runs.

\subsection{Dense Matrices}
In this setting, the matrix $V$ is dense, meaning most of its entries are non-zero.
For this experiment we fit a NMF model with reconstruction rank $r  = 20$, we compare the performance of the algorithms in terms of the relative error \ref{relative fro error} as a function of the number of iterations.
...
We fit a NMF model with reconstruction rank $r = 30$ using the MU algorithm, the HALS, the  ALS, and the PGD algorithms for $1000$ iterations.
...
In figure \ref{fig:error_vs_rank} the final relative reconstruction error is plotted against the reconstruction rank for 10 NMF models with increasing reconstruction rank using MU, HALS, ALS and PGD as solvers.

\section{The CBCL dataset}\label{CBCL dataset section}
In this section we use the CBCL face dataset used in \cite{Lee1999}, see section \ref{Example feature extraction NMF} for more details.
...
The tdt2\_top30 dataset is a benchmark dataset for performing topic detection, it was used in section \ref{Topic modeling section} as an example of the application of NMF in topic modeling.
...
In this case the results are similar to the ones obtained with the syntetic sparse data matrix \ref{fig:Sparse uniform data random init}: the lowest r
reconstruction error is achieved by the HALS algorithm, followed by ALS, PGD, and MU that are very close to each other.

\section{Summary}

In this chapter we compared the MU algorithm \ref{MU for beta}, the HALS algorithm \ref{HALS}, the ALS algorithm \ref{ALS} and the PGD algorithm \ref{Projected Gradient} using the 
relative reconstruction error \ref{relative fro error}. 
% We distinguished between sparse and dence matrices and observed that some algorithms performs differently in the two cases.
% While ALS performs poorly in the dense case, it seems to be much more efficient in the sparse case. The MU algorithm is the algorithm  with the slowest convergence,  and HALS is the fastest.
% The PGD is usually faster than MU, while reaching a minima with similar value.


For sparse matrices, ALS performs efficiently, while for dense matrices it converges poorly. HALS converges the fastest in both cases. MU has the slowest convergence. PGD converges faster than MU and reaches a similar minimum error.
MU is the slowest algorithm in terms of convergence.
\par
