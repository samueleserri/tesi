...
\subsubsection{Computational cost}
The computational cost discussion here is similar to the one presented for the MU algorithm with the Frobenius norm in section \ref{MU derivation section}, since the main cost is in the computation of the gradients. The total cost per iteration is $O(mnr)$ for each update of $W$ and $H$.
...
the cost is $O(mnr)$ per iteration. In practice HALS is generally faster than MU while having the same computational cost per iteration. See \cite{gillis2020nonnegative} 8.3.3.
...
For others values of $\beta$ we need to adjust MU updates by a scaling factor $\gamma(\beta)$ \cite{févotte2011algorithmsnonnegativematrixfactorization} defined as:
\begin{equation}
    \gamma(\beta) =
    \begin{cases}
...
however some second-order methods have been proposed in the literature, see \cite{huang2019low} and \cite{fu2020computing} for a general survey.
