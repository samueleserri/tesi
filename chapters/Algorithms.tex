...
In this chapter all the algorithms studied in this thesis and used in the numerical simulations of chapter \ref{Experiments chapter} are derived and formally presented, along with their convergence properties when available.
...
\par
The complete algorithm with the stopping criterion \ref{stopping criterion I} is summarized in the following pseudo-code \ref{Two block exact CD}:
\begin{algorithm}[ht]
...
\par
The algorithm can be summarized in the following pseudo-code \ref{NNDSVD}:
\begin{algorithm}[H]
...
\section{Projected methods}
    Since the non-negative least squares sub-problems of algorithm \ref{ANLS} are non trivial to solve, a possible approach is to solve the sub-problems without the non-negativity constraints and then project the solution onto the non-negative orthant.
