...
This is very easy to implement since most programming languages have built-in functions to solve least squares problems. For instance in MATLAB one can use the backslash operator `$\backslash$' to solve linear least squares problems. 
...
\par
In HALS the columns of $W$ and the rows of $H$ are updates ciclically, thus the update for $H[\ell,:]$ is performed by using the most recent update of $W[:,\ell]$ and the update for $W[:,\ell]$ is performed by using the most recent update of $H[\ell,:]$.
