...
This chapter describes all the algorithms studied in this thesis and used in the numerical simulations of chapter \ref{Experiments chapter}. For each algorithm we present the derivation and the convergence properties when available.
The algorithms are listed here: multiplicative updates (MU) with the Frobenius norm, with the Kullback-Leibler divergence and with the $\beta$-divergence, alternating non-negative least squares (ANLS), alternating least squares (ALS), projected gradient descent (PGD), hierarchical alternating least squares (HALS).
\par
...
Furthermore the optimization problem (\ref{NMF optimization problem}) is a constrained optimization problem, since the factors \( W \) and \( H \) are constrained to be non-negative, (\( W \geq 0 \) and \( H \geq 0 \)).
To handle the non-negativity constraints, the algorithms presented in this chapter use two main approaches: the first one is to design a scheme that ensures the non-negativity of the iterates at each step, this is the case of the MU, and ANLS.
The second approach is to first solve the subproblems without the non-negativity constraints and then project the solution onto the non-negative orthant, this is the case of the ALS, HALS, and PGD.

% \begin{table}
%     \centering
%     \caption{Summary of the NMF algorithms presented in this chapter.}
%     \label{tab:algo_summary}
%     \begin{tabular}{|l|c|}
%         \hline
%         \textbf{Algorithm} & \textbf{Reference} \\
%         \hline
%         MU (Frobenius) & Algorithm \ref{MU for Frobenius} \\
%         MU (Kullback-Leibler) & Algorithm \ref{MU for KL} \\
%         MU ($\beta$-divergence) & Algorithm \ref{MU for beta} \\
%         ANLS & Algorithm \ref{ANLS} \\
%         ALS & Algorithm \ref{ALS} \\
%         PGD & Algorithm \ref{Projected Gradient} \\
%         HALS & Algorithm \ref{HALS} \\
%         \hline
%     \end{tabular}
% \end{table}
...
\begin{algorithm}
    ...
    \end{algorithmic}
\end{algorithm}
This general scheme is widely used, and we present different ways to update the factors \( W \) and \( H \). There are mainly two observations that justify this approach:
\begin{enumerate}
    \item When one of the factors is fixed, the optimization subproblem is convex for most of the common cost functions. That makes it easier to design algorithms to solve it.
...
If the subproblems of Algorithm \ref{2-BCD} are solved exactly at each iteration, the method is called exact 2-BCD.  
The exact 2-BCD scheme is summarized in Algorithm \ref{Exact 2-BCD}.
\begin{algorithm}[ht]
...
\par
The complete algorithm with the stopping criterion (\ref{stopping criterion I}) is summarized in Algorithm \ref{Two block exact CD}.
\begin{algorithm}
    \caption{Two-blocks coordinate descent for NMF with stopping criterion \ref{stopping criterion I}}
    \label{Two block exact CD}
    \begin{algorithmic}[1]
...
The algorithms commonly used for NMF are iterative methods that require an initial guess for the factors \( W^{(0)} \) and \( H^{(0)} \). These algorithms are generally sensitive to the initialization.
...
\par
In this section, we present a slightly more sophisticated initialization method called non-negative double singular value decomposition (NNDSVD) \cite{boutsidis2008svd}.
Recall that the Eckart-Young theorem states that the best rank-$r$ approximation of a matrix $V$ in the Frobenius norm sense, is given by its truncated SVD \cite{schmidt1907theorie}; then the idea behind NNDSVD is to use the SVD of $V$ to compute a non-negative initialization for the factors \( W^{(0)} \) and \( H^{(0)} \).
\par
The algorithm is summarized in Algorithm \ref{NNDSVD}.
\begin{algorithm}
...
In this section we derive the MU algorithm for two different cost functions: the Frobenius norm and the Kullback-Leibler divergence.
In \cite{lee2000algorithms} Lee and Seung proved the convergence of Mu algorithm for the Frobenius and the Kullback-Leibler divergence.
A detailed derivation using matrix algebra can be found in \cite{burred2014detailed}.
In section \ref{MU beta section} we present the MU algorithm for the more general case of beta divergence \cite{févotte2011algorithmsnonnegativematrixfactorization}.
\subsection{MU with the Frobenius norm}
In this section, we derive the MU iteration scheme for the case where
...
\begin{equation}\label{re-scaled update}
    H \leftarrow H - \frac{H}{W^\top WH} \circ \nabla_H \frac{1}{2} \|V - WH\|_F^2.
\end{equation}
\begin{remark}
    Note that in this context the operator $\frac{A}{B}$ is the element-wise division of matrices $A$ and $B$, that is $\frac{A}{B}[i,j] = \frac{A[i,j]}{B[i,j]}$ for all $i,j$. % This operator is well-defined only if all elements of $B$ are strictly positive.
\end{remark}
Equation (\ref{re-scaled update}) leads to the following update rule:
\begin{equation}
...
\end{equation}
and a re-scaled gradient descent update for \( W \) is given by:
\begin{equation}
...
\end{equation}
\par
Recall that in subsection \ref{Gradient descent section} we have introduced the gradient descent as an iterative method for finding local minima with the following update rule: $x_{k+1} = x_k - \eta_k\nabla f(x_k)$.
...
\end{equation}
then selecting the step size as:
\begin{equation}
...
\end{equation}
simplifying, we can finally write the update rule for \( W \) as:
\begin{equation}
...
\end{equation}
\par The complete MU algorithm is summarized in the Algorithm \ref{MU for Frobenius}:
\begin{algorithm}
    \caption{Multiplicative Updates for NMF with the Frobenius norm}
    \label{MU for Frobenius}
    \begin{algorithmic}[1]
        \Require \( V \in \mathbb{R}_+^{m \times n} \), rank \( r \), initial factors \( W^{(0)} \in \mathbb{R}_+^{m \times r} \), \( H^{(0)} \in \mathbb{R}_+^{r \times n} \)
        % \output \( W \in \mathbb{R}_+^{m \times r} \), \( H \in \R_+^{r \times n} \) such that \( V \approx WH \)
        \While{stopping criterion not met}
...
\begin{theorem}[\cite{lee2000algorithms}, Theorem 1] \label{MU convergence Fro}
    The value of the objective function $D(V, WH) = \|V - WH \|_F^2$ is non-increasing when $W^{(k)}, H^{(k)}$ are produced by Multiplicative updates as in Algorithm \ref{MU for Frobenius}.
    Furthermore $W^{(k)}$ and $H^{(k)}$ are stationary points of the objective $D(V, WH)$ if and only if the distance is invariant under this iterative scheme.
\end{theorem}
\begin{proof}
...
The cost of the latter is $O(2mnr)$ while the cost of the former is $O(mr^2 + r^2 n) = O(r^2(m+n))$. And in most NMF applications $r \ll \min(m,n)$.
The second term $VH^\top$ requires $O(mnr)$ operations.
\subsection{MU with the Kullback-Leibler divergence}
In this section, we derive the MU iterative scheme for the case where
...
\end{equation}
Using the same reasoning as in the previous section, we can interpret the re-scaled gradient descent used in the MU updates as a gradient descent with a variable step size $\eta_k$ that is constrained to ensure the non-negativity of the iterates.
We derive the MU updates for $W$:
...
\end{equation}
The complete MU algorithm is summarized in Algorithm \ref{MU for KL}.
\begin{algorithm}
...
\begin{theorem}[\cite{lee2000algorithms}, Theorem 2] \label{MU convergence KL}
    The value of the objective function $D(V, WH) = D_{KL}(V \| WH)$ is non-increasing when $W^{(k)}, H^{(k)}$ are produced by multiplicative updates as in Algorithm \ref{MU for KL}.
    Furthermore $W^{(k)}$ and $H^{(k)}$ are stationary points of the objective $D(V, WH)$ if and only if the distance is invariant under this iterative scheme.
\end{theorem}
\begin{proof}
    The proof of this theorem can be found in \cite{lee2000algorithms} section 6.
\end{proof}
\subsubsection{Computational cost}
In the case of the Kullback-Leibler divergence, the computational cost of the MU algorithm is given by the computation of the gradients: $\frac{V}{WH} H^\top$ and $\textbf{1} H^\top$ for the update of $W$.
...
The alternating non-negative least squares (ANLS) method consists in solving exactly the non-negative least squares subproblems at each iteration of the two-blocks coordinate descent scheme (Algorithm \ref{2-BCD}), this is a case of exact 2-BCD (Algorithm \ref{Exact 2-BCD}).
...
\begin{equation}\label{ANLS subproblem I}
    W^{(k + 1)} = \arg\min_{W \geq 0} \|V - WH^{(k)}\|_F^2
\end{equation}
and
\begin{equation}\label{ANLS subproblem II}
    H^{(k + 1)} = \arg\min_{H \geq 0} \|V - W^{(k)}H\|_F^2.
\end{equation}
The subproblem (\ref{ANLS subproblem II}) can be decoupled into $n$ independent non-negative linear least squares problems.
That is, we can equally write the objective as:
\begin{align}
    &\|V - W^{(k)}H\|_F^2 = \sum_{j=1}^n \|V(:,j) - W^{(k)}H(:,j)\|_2^2
\end{align}
where $V(:,j)$ and $H(:,j)$ denote the $j$-th column of $V$ and $H$ respectively.
Then the update for $H$ can be computed by solving $n$ independent non-negative linear least squares problems of the form:
\begin{equation}
    H(:,j) = \arg\min_{h \geq 0} \|V(:,j) - W^{(k)}h\|_2^2, \quad \text{for } j = 1, \ldots, n.
\end{equation}
Analogously, the update for $W$ can be computed by solving $m$ independent non-negative linear least squares problems of the form:
\begin{equation}
    W(i,:) = \arg\min_{w \geq 0} \|V(i,:) - w H^{(k)}\|_2^2, \quad \text{for } i = 1, \ldots, m.
\end{equation}
However, solving exactly the constrained least squares subproblems is non-trivial; see \cite{kim2008nonnegative} for an example of an algorithm based on active-set methods to solve non-negative least squares problems.
The algorithm is summarized in Algorithm \ref{ANLS}.
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \While{stopping criterion not met}
            \For i = 1 to m
                \State \( W(i,:) \leftarrow \arg\min_{w \geq 0} \|V(i,:) - w H^{(k)}\|_2^2 \)
            \EndFor
            \For j = 1 to n
                \State \( H(:,j) \leftarrow \arg\min_{h \geq 0} \|V(:,j) - W^{(k)}h\|_2^2 \)
            \EndFor
...
\section{Projected methods}
    Since the non-negative least squares subproblems of Algorithm \ref{ANLS} are non trivial to solve, a possible approach is to solve the subproblems without the non-negativity constraints and then project the solution onto the non-negative orthant.
    ...
    \end{equation}
    Or equivalently, we can write the updates in the decoupled form as:
    \begin{equation}
        W(i,:) = \max\left(0, \arg\min_{w} \|V(i,:) - w H^{(k)}\|_2^2 \right), \quad \text{for } i = 1, \ldots, m.
    \end{equation}
and
\begin{equation}
    H(:,j) = \max\left(0, \arg\min_{h} \|V(:,j) - W^{(k)}h\|_2^2 \right), \quad \text{for } j = 1, \ldots, n.
\end{equation}
\subsection{Alternating least squares (ALS)}
The alternating least squares (ALS) method is a projected method where the subproblems are solved using the linear least squares solution (\ref{linear least sqaures equation}) and then projected onto the non-negative orthant.
This is very easy to implement since most programming languages have built-in functions to solve linear least squares problems. For instance in MATLAB one can use the backslash operator `$\backslash$'. 
In Python, that is the language used to create the repository associated with this thesis, the method \textit {`numpy.linalg.lstsq'} is used. % to solve linear least squares problems.
The ALS algorithm is summarized in Algorithm \ref{ALS}.
\par
...
Although the ALS algorithm is very popular in practice due to its simplicity, it does not have any convergence guarantees. See \cite{gillis2020nonnegative}, subsection 8.3.2 for an example.
\subsubsection{Computational cost}
The computational cost of the ALS algorithm is given by the cost of solving the least squares problems at each iteration.
Using the normal equations, the cost of solving the least squares problem for \( W \) is $O(mnr + mr^2 + r^3)$ and for \( H \) is $O(mnr + nr^2 + r^3)$. 
...
This method is discussed in detail in \cite{lin2007projected}.
\par
...
\begin{equation}
    \nabla_W \frac{1}{2} \|V - WH\|_F^2 = (WH - V) H^\top = W H H^\top - V H^\top.
\end{equation}
In the projected gradient algorithm, the step size is chosen to be the Lipschitz constant of the gradient.
...
\end{equation}
The complete projected gradient descent algorithm is summarized in Algorithm \ref{Projected Gradient}.
\begin{algorithm}
...
Since this is a gradient descent method with a Lipschitz step size with a projection onto the non-negative orthant when the iterates go out of bounds,
Theorem \ref{GD convergence theorem} can be applied to ensure the convergence of the projected gradient descent method for NMF to a stationary point of the objective function.
\subsubsection{Computational cost}
The complexity analysis here is similar to the one presented for the MU algorithm with the Frobenius norm in section \ref{MU derivation section}, since the main cost is in the computation of the gradients. The total cost per iteration is $O(mnr)$ for each update of $W$ and $H$.
...
\par
The main idea derives from the observation that for a fixed column $V[:,j] = v$ of the matrix \( V \), the optimization problem
\begin{equation}
    \min_{h} \| v - wh \|_2^2
\end{equation}
...
\end{align}
Note that in the unconstrained case, for a generic vector $v \in \R^m$, the optimal solution can also be negative, however we can apply the projection idea to obtain a non-negative solution.
...
\end{equation}
\par
Now we apply this idea to the NMF problem. Let $H[\ell,:]$ be the $\ell$-th row of the matrix \( H \) and $W[:,\ell]$ be the $\ell$-th column of the matrix \( W \);
the optimization problem for a fixed row $H[\ell,:]$ becomes:
...
\end{align}
this has the same form as the problem previously solved for a fixed column of \( V \), thus the solution is given by:
\begin{equation}
    H[\ell,j] = \max\left(0, \frac{W[:,\ell]^\top\tilde{R}_{\ell}[:,j]}{W[:,\ell]^\top W[:,\ell]} \right)
\end{equation}
which can be written in matrix form as:
\begin{equation}
    H[\ell,:] = \max\left(0, \frac{\tilde{R}_{\ell}^\top W[:,\ell]}{W[:,\ell]^\top W[:,\ell]} \right).
\end{equation}
...
\par
In HALS the columns of $W$ and the rows of $H$ are updates cyclically.
The complete HALS algorithm is summarized in Algorithm \ref{HALS}.
\begin{algorithm}
...
\subsubsection{Computational cost}
The computational cost of the HALS algorithm is essentially the same of the MU algorithm with the Frobenius norm discussed in section \ref{MU derivation section}, since the main cost is in the computation of the residual matrices $\tilde{R}_{\ell}$,
the cost is $O(mnr)$ per iteration. In practice HALS is generally faster than MU while having the same computational cost per iteration, (see \cite{gillis2020nonnegative}, subsection 8.3.3).
\section{MU with \texorpdfstring{$\beta$-divergence}{beta-divergence}} \label{MU beta section}
...
For the definition of the beta divergence see subsection \ref{Beta divergence section}.
\par
In this section, we derive the MU iteration scheme for $D(V, WH) = D_{\beta}(V, WH)$ the beta divergence in the general case.
...
However, since they are the most popular cost functions, we presented them separately.
...
\end{equation}
a re-scaled gradient descent update for \( W \) is then given by:
\begin{equation}
...
\end{equation}
Ultimately, the updates are given by:
\begin{equation}
...
\end{equation}
\par
The complete MU algorithm is summarized in the Algorithm \ref{MU for beta}.
\begin{algorithm}
...
For others values of $\beta$ we need to adjust the MU updates by a scaling factor $\gamma(\beta)$ \cite{févotte2011algorithmsnonnegativematrixfactorization} defined as:
\begin{equation}
    \gamma(\beta) =
    \begin{cases}
...
This result is obtained in \cite{févotte2011algorithmsnonnegativematrixfactorization} using the majorization-minimization framework.
\par
...
\end{theorem}
\begin{proof}
    The proof of this theorem can be found in \cite{gillis2020nonnegative} subsection 8.2.4.
\end{proof}
\subsubsection{Computational cost}
...
\begin{equation}
    \nabla_H D_{\beta}(V, WH) = W^\top \left( (WH)^{\beta - 1} - V \circ (WH)^{\beta - 2} \right)
\end{equation}
while the gradient with respect to \( W \) is given by:
\begin{equation}
    \nabla_W D_{\beta}(V, WH) = \left( (WH)^{\beta - 1} - V \circ (WH)^{\beta - 2} \right) H^\top.
\end{equation}
...
in this chapter we derived and presented the general framework for designing NMF algorithms,
and derived the most important ones, namely the multiplicative updates with beta divergence (Algorithm \ref{MU for beta}), which includes the MU for the Frobenius norm and the Kullback-Leibler divergence as special cases for $\beta = 2$ and $\beta = 1$ respectively, the alternating non-negative least squares method (Algorithm \ref{ANLS}), the alternating least squares method (Algorithm \ref{ALS}), the projected gradient descent method (Algorithm \ref{Projected Gradient}), and the hierarchical alternating least squares method (Algorithm \ref{HALS}).
There are different variants of the algorithms presented here, notably some algorithm can be accelerated, \cite{gillis2012accelerated}.
...
however, some second-order methods have been proposed in the literature, see \cite{huang2019low} for a general survey.
