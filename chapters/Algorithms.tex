...
In section \ref{initialization section} the sensitivity of NMF algorithms to the initialization in general is highlighted, and the non-negative Double Singular Value Decomposition (NNDSVD) initialization method is presented (Algorithm \ref{NNDSVD}).
\par
...
To handle the non-negativity constraints, the algorithms presented in this chapter use two main approaches: the first one is to design a scheme that ensure the non-negativity of the iterates at each step, this is the case of the MU, and ANLS;
...
\par
The algorithm can be summarized in Algorithm \ref{NNDSVD}.
\begin{algorithm}
...
\end{equation}
simplifying we obtain the update rule for \( W \):
\begin{equation}
...
\begin{theorem}[\cite{lee2000algorithms}, Theorem 1] \label{MU convergence Fro}
    ...
    Furthermore $W^{(k)}$ and $H^{(k)}$ are stationary points of the objective $D(V, WH)$ if and only if the distance is invariant under the iterative scheme (Algorithm \ref{MU for Frobenius}).
\end{theorem}
\begin{proof}
...
\begin{theorem}[\cite{lee2000algorithms}, Theorem 2] \label{MU convergence KL}
    The value of the objective function $D(V, WH) = D_{KL}(V \| WH)$ is non-increasing when $W^{(k)}, H^{(k)}$ are produced by multiplicative updates (Algorithm \ref{MU for KL}).
    Furthermore $W^{(k)}$ and $H^{(k)}$ are stationary points of the objective $D(V, WH)$ if and only if the distance is invariant under the iterative scheme (Algorithm \ref{MU for KL}).
\end{theorem}
\begin{proof}
    The proof of this theorem can be found in \cite{lee2000algorithms} section 6.
\end{proof}
...
The derivation of this decoupling is as follows:
\begin{align}
