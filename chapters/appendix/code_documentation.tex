...
This chapter concludes showing some examples on how the code can be used.
...
pip install -e .
\end{lstlisting}
this will install all required dependencies as well; this command uses the toml file \texttt{pyproject.toml}.
To verify the installation, you can run:
\begin{lstlisting}
pip list
\end{lstlisting}
to see the installed packages. You should see a package named \texttt{NMF} and its dependencies.
\newpage\section{Repository structure}\label{Repo structure section}
...
\begin{itemize}
    ...
    \item \texttt{init}: Initialization method for factor matrices W and H. Supported values:
        ...
        \item "nndsvda": NNDSVD with zeros replaced by the average of the input matrix.
        \item "custom": the user must set W and H manually.
    \end{itemize} (default random)
    \item \texttt{random\_state}: random seed for reproducibility. (default: None)
    ...
    \item \texttt{time\_limit}: time limit for the fitting process in seconds. (default: 30 seconds).
\end{itemize}
Public Methods:
\begin{itemize}
    ...
    \item \texttt{plot\_time\_vs\_error()}: plots the normalized reconstruction error on a logarithmic y-scale against time.
...
the older release did not have full compatibility, for more information visit \url{https://docs.scipy.org/doc/scipy/reference/sparse.migration_to_sparray.html}} interface.
...
\end{lstlisting}
to fit the model, we can simply call the fit method with the desired solver, in this example we use the multiplicative updates (MU) algorithm and the Kullback-Leibler divergence ($\beta$ = 1):
\begin{lstlisting}
...
\end{lstlisting}
Suppose now that you want to add a regularization term to the Frobenius norm, for instance the $\ell_1$ norm of the factors, then your objective becomes:
\begin{equation*}
  D(V, WH) = \|V - WH\|_F^2 + \alpha_W \|W\|_1 + \alpha_H \|H\|_1
\end{equation*}
fitting this model is straightforward: we instantiate a new model from \texttt{RegularizedNMF} specifying the values for $\alpha_W$ and $\alpha_H$, with the desired regularizer, in this case we pass the string \texttt{"ell\_1"} because we want to use the $\ell_1$ norm of the factors as regularization term.
\begin{lstlisting}
...
\end{lstlisting}
this method returns a dictionary where to the key ``W'' corresponds the matrix $W$ and to the key ``H'' corresponds the matrix $H$.
\subsection{One more example: performing feature extraction from the CBCL dataset} 
\subsubsection{Learning part of faces from the CBCL dataset}
Here we talk again about the example presented in subsection \ref{Example feature extraction NMF}; recall that this example was originally presented in the seminal paper on NMF by Lee and Seung \cite{Lee1999}. 
In this example, we reproduce the results using the package \texttt{NMF} with the MU algorithm and the Kullback-Leibler divergence.
\par
...
\end{lstlisting}
then, we need to load the dataset, to do so we define the following function:
\begin{lstlisting}
