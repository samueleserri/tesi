\documentclass[12pt]{book}

% Page layout
\usepackage[a4paper,margin=3cm]{geometry}

% Encoding and language
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{newunicodechar}
\newunicodechar{ℓ}{\ensuremath{\ell}}
\usepackage[english]{babel}
\usepackage{lmodern}
\usepackage{microtype}

% Math and graphics
\usepackage{amsmath,amssymb}
\usepackage{graphicx}
\usepackage{tikz}
\usepackage{float}
\usepackage{adjustbox}
\usetikzlibrary{arrows.meta, positioning, calc, 3d}
\usepackage{caption} 
\usepackage{subcaption}   
\usepackage{courier} 
% Tables
\usepackage{longtable}
% code
\usepackage{listings}
\lstset{language=Python,
        basicstyle=\ttfamily\small,
        keywordstyle=\color{blue},
        commentstyle=\color{green!60!black},
        stringstyle=\color{orange},
        showstringspaces=false,
        numbers=left,
        numberstyle=\tiny,
        stepnumber=1,
        numbersep=5pt,
        breaklines=true,
        frame=single,
        tabsize=4,
        captionpos=b,
        morekeywords={self, None, True, False, pip, cd, source, install, list, clone, np, numpy, fit, get_final_error}
}
% pseudocode
\usepackage{algorithm}
\usepackage{algpseudocode}
% Links
\usepackage{csquotes}
\usepackage{hyperref}
\hypersetup{
    hidelinks,
    colorlinks=true,
    linkcolor=black,
    urlcolor=black,
    citecolor=green,
    filecolor=black,
    breaklinks=true  
}

% add this to make page numbers always bottom center
\usepackage{fancyhdr}
\setlength{\headheight}{14.49998pt}  % fix fancyhdr warning
\pagestyle{fancy}
\fancyhf{}                      % clear all header/footer fields
\fancyhead[L]{\leftmark}        % chapter name in left header
\fancyhead[R]{\thepage}         % page number in right header
\renewcommand{\headrulewidth}{0.4pt}  % thin line under header
\renewcommand{\footrulewidth}{0pt}

% customize the plain page style (used on chapter-start pages)
\fancypagestyle{plain}{%
  \fancyhf{}                    % clear all header/footer fields
  \fancyfoot[C]{\thepage}       % page number centered in footer only
  \renewcommand{\headrulewidth}{0pt}  % no header line
  \renewcommand{\footrulewidth}{0pt}
}

\usepackage[backend=bibtex,style=numeric-comp,sorting=nyt,backref=true]{biblatex} % use bibtex backend to avoid biber
\addbibresource{references.bib} % The file containing the bibliography
% Simple metadata
\newcommand{\thesistitle}{TITOLO}
\newcommand{\thesisauthor}{Samuele Serri}
\newcommand{\thesismatricola}{SM3201369}
\newcommand{\thesissupervisor}{\'Angeles Mart\'{\i}nez Calomardo}
\newcommand{\thesisyear}{2024--2025}

% useful math commands
\newcommand{\R}{\mathbb{R}}
\newcommand{\N}{\mathbb{N}}

% create math environment for theorems, definitions, etc.
\usepackage{amsthm}
\newtheorem{definition}{Definition}[chapter]
\newtheorem{theorem}{Theorem}[chapter]
\newtheorem{lemma}{Lemma}[chapter]
\newtheorem{proposition}{Proposition}[chapter]
\newtheorem{corollary}{Corollary}[chapter]
\theoremstyle{remark}
\newtheorem{observation}{Observation}[chapter]
\newtheorem{terminology}{Terminology}[chapter]
\newtheoremstyle{boldremark}
  {\topsep}{\topsep}
  {}{}
  {\bfseries}{:}
  { }
  {}
\theoremstyle{boldremark}
\newtheorem{remark}{Remark}[chapter]    
\theoremstyle{remark}
\newtheorem{example}{Example}[chapter]

\setcounter{tocdepth}{2}

% Begin document


\begin{document}
\begin{center}
\begin{tikzpicture}[remember picture,overlay]
\node[anchor=center, inner sep= 0pt] 
{\includegraphics[width=.5\textwidth]
{logo_centenario-eps-converted-to.pdf}};
\centering
\end{tikzpicture}
\vfill
\LARGE
\textsc{Università degli Studi di Trieste}\\
\rule{\textwidth}{0.1mm}\\
\large
\textsc{Dipartimento di Matematica Informatica e Geoscienze -- MIGE}\\ 
\bigskip
Corso di Laurea in Intelligenza Artificiale e Data Analytics \\
%\small{Classe ministeriale: L-31 - Scienze e tecnologie informatiche}
\vfill %%\vspace{3cm}
\large
\textsc{Tesi di laurea triennale}\\
\vfill %%\vspace{3cm}
\LARGE
\textbf{Exploring Non-Negative Matrix Factorization: Algorithms and Applications}\\
\end{center}
\vfill
%%%%%%
%%%%%% Qui va inserito il nome del laureando
\begin{tabular}[t]{l}
Laureando:\\
\textbf{Samuele Serri}\\
\textsc{Matricola: SM3201369}
\end{tabular}
%%%%%%
\hfill 
%%%%%% Qui va inserito il nome del relatore
\begin{tabular}[t]{l}
Relatore: \\
\textbf{Prof. \'Angeles Mart\'{\i}nez Calomardo} \\
\end{tabular}
%%%%%%
\vfill
\begin{center}
\normalsize
\rule{8cm}{0.1mm}\\
\bigskip
ANNO ACCADEMICO 2024--2025
\end{center}
\thispagestyle{empty}  


\clearpage              
\pagenumbering{roman}
\shipout\null
\include{chapters/Acknowledgments} 
\tableofcontents\addcontentsline{toc}{chapter}{Table of Contents}
\hypersetup{linkcolor=red} % restore red for other \ref links
\bigskip
% pagina pdflatex -interaction=nonstopmode main.texvuota


\clearpage
% notation page not numbered
\thispagestyle{empty}
\include{chapters/Notation}
\thispagestyle{plain} % use plain style for first page of main matter
\null
\clearpage
\mainmatter
\pagenumbering{arabic}
\include{chapters/Introduction}
\include{chapters/Background}
\include{chapters/NMF}
\include{chapters/Algorithms}
\include{chapters/Experiments}

\appendix
\hypersetup{linkcolor=black} % make TOC links black
\listoffigures
\listofalgorithms
\listoftables
\hypersetup{linkcolor=red} 
\include{chapters/appendix/code_documentation}
\include{chapters/appendix/additional_plots}
\hypersetup{linkcolor=black} 

% Bibliography placeholder:
\printbibliography
\addcontentsline{toc}{chapter}{Bibliography }

\end{document}


