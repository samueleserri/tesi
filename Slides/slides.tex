...
\begin{frame}{Definition of non-negative matrix factorization (NMF)}
...
\begin{frame}{}%Make Titles Informative.}
  ...
  \begin{frame}{Example: learning parts of objects with NMF \small{(Lee \& Seung \cite{lee1999})}}
    \begin{columns}
      ...
      \column{0.5\textwidth}
      \begin{itemize}
        \item CBCL face dataset $\rightarrow$ 2429 images of faces with 19x19 pixels resolution.
        \item NMF with $r = 49$ $\rightarrow$ 49 basis images and corresponding coefficients.
        \item Columns of W $\rightarrow$ basis images that extract \alert{facial features}.
        \item Columns of $H$ $\rightarrow$ weight of each feature for each image.
        \item $v_i \approx \displaystyle\sum_{k=1}^r w_k h_{k,i}$.
      \end{itemize}
    \end{columns}
  \end{frame}
...
\textbf{MU can be interpreted as a scaled \alert{gradient descent} method:}
...
\begin{frame}{Numerical Simulations on the CBCL dataset}
  \begin{columns}
    \column{0.5\textwidth}
    \begin{figure}
    ...
    \begin{block}{\small Results}
      ...
      ALS performs the worst, with the highest time per iteration and lowest number of iterations.
      PGD has a higher time per iteration than MU and HALS, but is faster than ALS.
...
\begin{frame}[allowframebreaks]
  \frametitle<presentation>{Further Reading and References}
